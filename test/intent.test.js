...
test("parses budget, fastest priority, date window, and Tokyo openness", () => {
  const intent = parseTripIntent("Find me the fastest route from Chiang Mai or Bangkok to Redmond Aug 1-5, maybe Tokyo if worth it, under $1300");
...
test("parses one-way plus-minus date windows", () => {
  const intent = parseTripIntent("one way CNX to RDM plus or minus 3 days from August 1st, stopover Tokyo 1 night if worth it");
  assert.equal(intent.interpreted.tripType, "one-way");
